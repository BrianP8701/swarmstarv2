...
const AiMessage: React.FC<AiMessageProps> = ({ content }) => (
  <div className='self-center w-[90%] max-w-[700px]'>
    <div className='text-left  rounded-3xl p-3'>{content}</div>
  </div>
)
        ...
        updateQuery: (prev: FetchChatQuery, { subscriptionData }) => {
          ...
          return {
            ...
            chat: prev.chat
              ? {
                  ...prev.chat,
                  messages: [...(prev.chat.messages?.filter(Boolean) || []), newMessage].filter(
                    (message): message is NonNullable<typeof message> => message !== null && message !== undefined
                  ),
                }
              : null,
          }
