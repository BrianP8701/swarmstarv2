...
export function CreateSwarmDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const isSpawnButtonDisabled =
    !createSwarmRequest.informationGraphId || !createSwarmRequest.title || !createSwarmRequest.goal
        ...
        <div className='grid w-full items-start h-full overflow-auto gap-6 p-2 mt-4'>
          <div className='grid grid-cols-2 gap-6'>
            ...
            <SelectWithCreate
              ...
              options={
                user?.informationGraphs?.map(informationGraph => ({
                  value: informationGraph.id,
                  label: informationGraph.title ?? 'Untitled Information Graph',
                })) ?? []
              }
              onSelect={value => {
