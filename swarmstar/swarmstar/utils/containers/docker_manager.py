...
class DockerContainerManager(ContainerManagement):
    ...
    def start(self, image_id: str = "python:latest") -> str:
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to start terminal session due to Docker API error: {e}"
            )
        except Exception as e:
            raise Exception(
                f"An unexpected error occurred while starting terminal session: {e}"
            )
        return container.id

    def start_with_github(
        self, github_link: str, image_id: str = "python:latest"
    ) -> tuple:
        """
        ...
        for command in commands:
            try:
                ...
                exit_code, output = container.exec_run(
                    cmd=["bash", "-c", full_command], workdir="/repo"
                )

                output_decoded = output.decode("utf-8").strip()
                if exit_code != 0:
                    outputs.append(
                        f"Error: Command '{full_command}' failed with exit code {exit_code}. Output: {output_decoded}"
                    )
                else:
                ...
                raise ConnectionError(
                    f"Failed to send command due to Docker API error: {e}"
                )
            except Exception as e:
                raise Exception(
                    f"An unexpected error occurred while sending command '{command}': {e}"
                )
        return outputs
    ...
    def close(self, container_id: str) -> None:
        try:
            ...
            raise ConnectionError(
                f"Failed to close terminal session due to Docker API error: {e}"
            )
        except Exception as e:
            raise Exception(
                f"An unexpected error occurred while closing terminal session: {e}"
            )
