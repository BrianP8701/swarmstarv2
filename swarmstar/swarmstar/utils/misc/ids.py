...
async def generate_id(
    db: Database, table: DatabaseTableEnum, swarm_id: Optional[str] = None
) -> str:
    """
    ...
    database_map_entry = DATABASE_MAP[table]
    
    count_column_name = database_map_entry.get("count_column")
    identifier = database_map_entry.get("abbreviation")
    
    if count_column_name is not None and identifier is not None:
        if swarm_id is None:
        ...
        async with db.get_session() as session:
            ...
            await db.update(
                SwarmstarSpaceModel, swarm_id, {count_column_name: str(x + 1)}, session
            )
    else:
...
async def get_swarm_object_count(
    db: Database,
    swarm_id: str,
    ...
    """
    from data.constants import TABLE_ENUM_TO_SWARMSTAR_SPACE_COUNT_COLUMN
    ...
    async with session as session:
        result = await db.select(
            SwarmstarSpaceModel, swarm_id, [count_column_name], session
        )
        return result[count_column_name]


async def get_all_swarm_object_ids(
    db: Database,
    swarm_id: str,
    table: DatabaseTableEnum,
    session: Optional[AsyncSession] = None,
...
def get_table_enum_from_id(id: str) -> DatabaseTableEnum:
    from data.constants import TABLE_ABBREVIATION_TO_ENUM
...
def get_table_name_from_id(id: str) -> str:
    from data.constants import TABLE_ENUM_TO_TABLE_NAME

    table_enum = get_table_enum_from_id(id)
    return TABLE_ENUM_TO_TABLE_NAME[table_enum]


def get_operation_class_from_id(id: str) -> Type[BaseOperation]:
    from data.constants import TABLE_ENUM_TO_OBJECT_CLASS

    table_enum = get_table_enum_from_id(id)
    operation_class = TABLE_ENUM_TO_OBJECT_CLASS[table_enum]

    if not issubclass(operation_class, BaseOperation):
        raise TypeError(f"Class {operation_class} is not a subclass of BaseOperation")
