...
from swarmstar.instructors.instructors.plan_instructors.parallel_plan_instructor import (
    ParallelPlanInstructor,
)
from swarmstar.instructors.instructors.plan_instructors.review_parallel_plan_instructor import (
...
class ParallelPlan(BaseActionNode):
    ...
    context: ParallelPlanContext = ParallelPlanContext()
    ...
    async def generate_plan(self) -> List[SpawnOperation]:
        ...
        review_plan = await ReviewParallelPlanInstructor.review_plan(
            self.goal, parallel_plan.plan, self.get_most_recent_context(), self.id
        )
        ...
        if review_plan.confirmation:
            ...
            self.context.parallel_plan_review_feedback_history.append(
                review_plan.feedback
            )
