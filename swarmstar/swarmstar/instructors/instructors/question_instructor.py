...
class QuestionInstructor(BaseInstructor):
    questions: List[str] = Field(
        description="It seems like we need more information to do this. Ask questions to get more information. "
    )
    context: str = Field(
        description="If applicable, provide context for the questions."
    )
    ...
    def write_instructions(
        ...
        return [
            ...
            Message(
                ...
                content=f"What we need to do: {content}"
    ...
    async def ask_questions(
        ...
        response = await cls.get_client().instruct(
            messages=instructions, instructor_model=cls, action_node_id=action_node_id
        )
        return response
