...
class RouterInstructor(BaseInstructor):
    best_option: Optional[int] = Field(
        description="The index of the best option. None if there is no viable option."
    )
    unviable_options: List[int] = Field(
        description="The indices of the options that are unviable"
    )

    @classmethod
    def write_instructions(
        cls, options: List[str], content: str, system_message: str
    ) -> List[Message]:
        formatted_options = cls._format_options(options, content)
        return [
            Message(role=MessageRoleEnum.SYSTEM, content=f"{system_message}\n\n"),
            Message(
    ...
    def _format_options(cls, options: List[str], prompt: str) -> str:
        return "Prompt: {}\nOptions:\n{}".format(
            prompt,
            "\n".join(
                ["{}. {}".format(i + 1, option) for i, option in enumerate(options)]
            ),
        )
