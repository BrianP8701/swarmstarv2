...
class ReviewParallelPlanInstructor(BaseInstructor):
    ...
    feedback: str = Field(..., description="Provide feedback on why the subgoals cannot be executed in parallel. This feedback should be actionable and specific to the subgoals that need to be revised.")

    @staticmethod
    def write_instructions(goal: str, subgoals: List[str], context: Optional[str] = None) -> List[Message]:
        return [
            Message(
                role=MessageRoleEnum.USER,
                content=f"The subgoals are: {subgoals}. Please analyze if they can be executed in parallel." + (f"\n\nContext: {context}" if context else "")
            )
        ]
