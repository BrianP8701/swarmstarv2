...
class InstructorClient:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.aclient = cast(
                AsyncOpenAI, instructor.patch(AsyncOpenAI(api_key=OPENAI_KEY))
            )
    ...
    async def instruct(
        ...
        response_message = Message(
            role=MessageRoleEnum.ASSISTANT, content=completion.model_dump_json()
        )

        if logging and action_node_id is not None:
            await cls._log_instructor_call(
                messages + [response_message], action_node_id
            )

        return completion

    @staticmethod
    async def _log_instructor_call(
        messages: List[Message], action_node_id: str
    ) -> None:
